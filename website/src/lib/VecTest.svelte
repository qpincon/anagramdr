...
onMount(() => {
    ...
    context.fillText("AV", 0, 10);
    let pixels = ndarray(context.getImageData(0, 0, canvasElement.width, canvasElement.height).data, [canvasElement.height, canvasElement.width, 4])
    pixels = pixels.pick(-1, -1, 0).transpose(1, 0);
    console.log(pixels);    
    const complex = surfaceNets(pixels, 128);
    console.log(complex);

    var svgFile = ['<svg xmlns="http://www.w3.org/2000/svg" width="600" height="300">']
    ...
    elem.innerHTML = svgFile.join("");

    // const res = vectorizeText("AV", canvasElement, canvasElement.getContext('2d'), {
    //     width: 500,
...
<canvas bind:this={canvasElement} width=400 height=100></canvas>
